           ...
           <template v-if="item.children.length===1 && !item.children[0].children && !item.alwaysShow">
               <el-menu-item :index="item.path+'/'+item.children[0].path" class='submenu-title-noDropdown'>
                   <router-link :to="item.path+'/'+item.children[0].path">
                       <i class="icon" v-if="item.children[0].meta && item.children[0].meta.icon" :class="[item.children[0].meta.icon]"></i>
                       <span v-if="item.children[0].meta && item.children[0].meta.title">{{item.children[0].meta.title}}</span>
                   </router-link>
               </el-menu-item>
           </template>
            <el-submenu v-else :index="item.name || item.path" :key="item.name" class="bb">
                <template slot="title">
